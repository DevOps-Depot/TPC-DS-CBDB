...
function gen_data() {
  ...
  for i in $(psql -v ON_ERROR_STOP=1 -q -A -t -c "${SQL_QUERY}"); do
    ...
    echo "ssh -n -f ${EXT_HOST} \"bash -c 'cd ~/; ./generate_data.sh ${GEN_DATA_SCALE} ${CHILD} ${PARALLEL} ${GEN_DATA_PATH} > ${GEN_DATA_PATH}/tpcds.generate_data.${CHILD}.log 2>&1 &'\""

    ssh -n -f ${EXT_HOST} "bash -c 'cd ~/; ./generate_data.sh ${GEN_DATA_SCALE} ${CHILD} ${PARALLEL} ${GEN_DATA_PATH} > ${GEN_DATA_PATH}/tpcds.generate_data.${CHILD}.log 2>&1 &'" &
  done
  wait
}
